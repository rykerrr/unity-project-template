...
namespace _Project.Scripts.Editor
{
    public class EditorUtils : MonoBehaviour
    {
        ...
        {
            EditorWindow
                inspectorToBeLocked = EditorWindow.mouseOverWindow; // "EditorWindow.focusedWindow" can be used instead
        ...
        [MenuItem("Tools/Toggle GameObject Active State &e")]
        static void ToggleSelectedObjectActiveState()
        {
            var selection = Selection.transforms;
            if (selection.Length <= 0) return;
            
            foreach (var transf in selection)
            {
                var go = transf.gameObject;
                
                if (go.scene.IsValid() && go.scene == SceneManager.GetActiveScene())
                {
        ...
        static void ToggleInspectorDebug()
        {
            ...
            if (targetInspector != null && targetInspector.GetType().Name == "InspectorWindow")
            {
                ...
                InspectorMode
                    mode = (InspectorMode) field.GetValue(targetInspector); //read the value for our target inspector
